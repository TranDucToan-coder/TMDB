...
import { RenderDetail_Celebritties } from "../DetailItem/Detail_Celebrity"
import { RenderDetail_Trailer } from "./renderDetail_Trailer"
...
export const RenderCaseroulTrailer = ({ data }: { data: ResponseVideo }) => {
    ...
    return (
        <Carousel>
            <CarouselContent>
                {data && data?.map((item : Video) => (
                    <CarouselItem className="basis-full h-70 sm:h-150" key={item.key}>
                        <RenderDetail_Trailer key={item.key}/>
                    </CarouselItem>
                ))}
            </CarouselContent>
            <CarouselPrevious/>
...
export const RenderCaseroulCelebrities = ({ data }: { data: ResponseActor }) => {
    return (
        <Carousel>
            <CarouselContent>
                {data ? data?.results.map((item : Actor) => (
                    <CarouselItem key={item.id} className="basis-1/2 sm:basis-1/5">
                        <RenderDetail_Celebritties profile_path={item.profile_path} name={item.name}/>
                    </CarouselItem>
                )) : (<p>Loading</p>)}
            </CarouselContent>
            <CarouselPrevious/>
